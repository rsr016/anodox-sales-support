  ...
  <div>
    <div v-if="!exiting" class="grid grid-cols-12 grid-flow-col mt-5 mb-6">
        ...
        <UTable :rows="dataRows" :columns="dataColumns" :ui="dataTableConfig">
          <template #value-data="{ row }">
            <UInput
              ...
              v-if="row.parameter == 'Capacidade (kWh)'"
            />
            <div v-else>
              {{ row.value }}
            </div>
      ...
      <div class="md:flex hidden col-span-4 container">
        <NuxtImg
          src="/HiPower_ALSES_XL.webp"
          sizes="200px"
          class="place-self-center"
        />
        <p class="font-semibold text-gray-900 place-self-center">
          Equipamento sugerido: ALSES-2150-1000
        </p>
      </div>
    </div>
  </div>
...
const dataRows = computed(() => {
  return [
    ...
    {
      parameter: "RTE (%)",
      value: 100 - props.project.discharge_losses - props.project.charge_losses,
    },
    { parameter: "Max SoC (%)", value: props.project.max_soc },
    { parameter: "Min SoC (%)", value: props.project.min_soc },
    {
      parameter: "Ciclos Usados no Período",
      value: cycles_in_period.value,
      class: "bg-green-500/50",
    },
    {
      parameter: "Máxima Potência Requerida (kW)",
      value: max_power_in_period.value,
      class: "bg-green-500/50",
    },
  ];
});
...
const max_power_in_period = computed(() => {
  return Math.max.apply(
    Math,
    props.data.map((x) => x.service_from_bess)
  );
});
</script>
